...
public class MyHandler : MonoBehaviour
{
    ...
    IEnumerator move()
    {
        ...
        RiseLeaves.SetActive(true);
        shadow.SetActive(false);


        unitychan.transform.localPosition = new Vector3(0, -1.5f, 0);
        unitychan.transform.localRotation = Quaternion.Euler(42, 40, 5);


        while (unitychan.transform.localPosition != new Vector3(0, 0.8f, 0))
        {
            yield return new WaitForEndOfFrame();
            unitychan.transform.localPosition = Vector3.MoveTowards(unitychan.transform.localPosition, new Vector3(0, 0.8f, 0), 1f * Time.deltaTime);
            if (unitychan.transform.localPosition.y >= 0.6)
            {
                shadow.SetActive(true);
                RiseLeaves.SetActive(false);
            }
        }
    }
